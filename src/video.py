...
def generate_cover(title: str, output_path: str, script: str = "") -> str:
    ...
    prompt = (
        ...
        f"Their faces are distinct from each other and each subtly resembles a famous thinker in facial features only: "
        f"Left (Aristo): face subtly resembles Aristotle — broad forehead, short beard, calm wise eyes. "
        f"Center (Albert): face subtly resembles Einstein — bushy eyebrows, expressive eyes, wild hair. "
        f"Right (Isaac): face subtly resembles Newton — long nose, sharp focused gaze, straight hair. "
